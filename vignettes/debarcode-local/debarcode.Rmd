...
params:	
  ...
  dir_out_debarcode: "/scratch/rdxmig002/data/cytof/fcs/debarcode"
  dir_out_compensate: "/scratch/rdxmig002/data/cytof/fcs/compensate"
  trans_out: true
  fixed: true
---
...
```{r out_fcs, eval = !is.null( params$dir_out_debarcode) }
if( params$trans_out ){
  ex <- exprs(re)
  ex <- 5 * sinh(ex)
  re@exprs <- ex
...
dir_out_debarcode <- ifelse( params$fixed, 
							 paste0( params$dir_out_debarcode, "-fixed_cutoff" ), 
				             paste0( params$dir_out_debarcode, "-flexible_cutoff" ) )
outFCS( x = re, y = fr, out_path = dir_out_debarcode, out_nms <- out_nms ) 
if( params$trans_out ){
  ex <- asinh(ex/5)
  re@exprs <- ex
}
...
```{r computeSpillmat, eval = !is.null( params$dir_out_compensate )}
spillMat <- computeSpillmat(x=re)
```

```{r plotSpillmat, eval = !is.null( params$dir_out_compensate )}
bc_ms <- c( 137,138, 140, 161:164, 166:168, 170, 
151, 153, 144, 155, 156, 158, 160, 165, 
127, 
139, 175:176, 142:146, 148, 150, 
108, 141, 147, 149, 152, 
154, 159, 169, 89, 171:174 )
bc_ms <- c( 137,138, 209, 140, 161:164, 166:168, 170, 
151, 153, 144, 155, 156, 158, 160, 165, 
127, 113, 115, 191, 193, 
139, 175:176, 142:146, 148, 150, 
108, 141, 198, 103,  147, 149, 152, 
154, 159, 169, 89, 171:174 )
