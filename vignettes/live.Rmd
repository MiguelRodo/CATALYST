...
if( params$cluster ){
  ...
  if( !is.null( params$ind ) ) path_fcs_file_short_vec <- path_fcs_file_short_vec[params$ind]
  if( !params$ncfs ){
	...
	if( !is.null( params$ind ) ){
		  fn <- stringr::str_sub( path_fcs_file_short_vec, end = -5 )
		  path_ncfs_load <- file.path( path_pkg, "ncfs", "debeaded", fn )	
		} else{
		  path_ncfs_load <- file.path( path_pkg, "ncfs", "debeaded" )		
		}
...
if( !is.null( params$ind ) ){
	  fn <- stringr::str_sub( path_fcs_file_short_vec, end = -5 )
	  fn <- stringr::str_replace( fn, "debeaded", "singlets" )
	  path_ncfs_save <- file.path( path_pkg, "ncfs", "singlets", fn )	
	} else{
	  path_ncfs_save <- file.path( path_pkg, "ncfs", "singlets_all" )		
	}
