...
i <- 1
path_fcs_folder <- "C:/Users/migue/OneDrive - University of Cape Town/Work/PhD/Data/Missing Normalised CyTOF Data"
path_fcs_file_short_vec <- list.files(path_fcs_folder, pattern = "fcs")
...
dir_fcs_out <-  file.path(path_fcs_folder, "debeaded")
if(!dir.exists(dir_fcs_out)) dir.create(dir_fcs_out, recursive = TRUE)
dir_html <- file.path(path_fcs_folder, "processing files", 
                "debeading")
if(!dir.exists(dir_html)) dir.create(dir_html, recursive = TRUE)
fn_html <- paste0(str_sub(path_fcs_file_short_vec[i], end = -4), "html")
fs <- removeBeads(x = fr, y = "dvs", trans = TRUE, 
                  out_path = dir_fcs_out, 
                  fn = str_sub(path_fcs_file_short_vec[i], end = -5))
